...
if not st.session_state.signed_in:
    ...
    with col2:
        ...
        with tab1:
            ...
            google_auth_url = get_google_auth_url()
            url = "https://accounts.google.com/o/oauth2/auth?" + urlencode(params)
            google_btn = f"<a href='url' target='_self' style='display:block;text-align:center;padding:12px;background:#ffffff;color:#050d1a;border-radius:6px;font-family:Rajdhani,sans-serif;font-weight:700;font-size:15px;letter-spacing:2px;text-decoration:none;'>🔵 SIGN IN WITH GOOGLE</a>"
            st.markdown(google_btn, unsafe_allow_html=True)
